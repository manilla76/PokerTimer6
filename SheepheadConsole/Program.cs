...
namespace SheepheadConsole
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            Game game = new Game(new Deck(), new Team());  // could be using injection
            while (continueGame)
            {
                game.StartHand();
                // start hand
                ...
                do
                {
                    ...
                    {
                        // start round 
                        // show cards of first player
                        // show cards already played
                        ...
                        Console.WriteLine($"Current Trick: {game.CurrentTrick.ShowTrick()}"); 
                    ...
                    } while (game.CurrentTrick.CardsPlayed < 4);  // each player plays
                    // process trick
                    game.ProcessTrick();
                    game.CurrentTrick = new Trick();
                } while (game.TrickList.Count < 8);   // repeat round until done
                game.FinishHand(); // finish hand
            }
        }
